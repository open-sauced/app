...
const Feeds = () => {
  ...
  const singleHighlight = data && data.length > 0 ? data.filter((highlight) => highlight.id === id) : undefined;
  ...
  return (
    <>
      {singleHighlight && singleHighlight.length > 0 ? (
        <div className="container  mx-auto px-2 md:px-16 gap-12 lg:justify-end pt-12 flex flex-col md:flex-row">
          <Dialog open={true}>
            <DialogContent className="sm:max-w-[650px] max-h-[550px] overflow-scroll">
              ...
              <div className="mt-10 flex gap-8 flex-col ">
                ...
                {singleHighlight &&
                  singleHighlight.length > 0 &&
                  // eslint-disable-next-line camelcase
                  singleHighlight.map(({ id, url, title, created_at, highlight, name, login }) => (
