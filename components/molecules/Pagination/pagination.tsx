...
const Pagination = ({
  ...
  const middlePages = Array.from({ length: Math.min(pages.length, pageSize) }, (_, index) =>
    currentPage <= Math.floor(pageSize / 2)
      ? index + 1
      : currentPage >= pages.length - Math.floor(pageSize / 2)
      ? pages.length - pageSize + index + 1
      : currentPage - Math.floor(pageSize / 2) + index
      ...
      <div className="w-max flex gap-x-4 items-center">
        ...
        {goToPage && (
          <div className="hidden md:block">
            <PaginationGotoPage totalPage={totalPage} page={currentPage} setPage={handleSelected} name={""} />
          </div>
        )}
      </div>
    </>
