...
const ReportsHistory = ({ reportList, repositories }: ReportsHistoryProps): JSX.Element => {
  ...
  return (
    <div className="flex flex-col bg-white border rounded-lg overflow-hidden">
      <div className="flex justify-between px-3 md:px-6 py-3 rounded-t-lg bg-light-slate-3">
      ...
      <div className="flex flex-col justify-between w-full">
        {reportList &&
          reportList.map((report, index) => {
            ...
            return (
              <div className="flex items-center py-3 px-3 md:px-6 odd:bg-white even:bg-slate-50" key={index}>
        ...
        {!reportList || (reportList.length === 0 && <Text className="py-3 px-6">Currently there are no reports</Text>)}
      </div>
    </div>
  );
};
