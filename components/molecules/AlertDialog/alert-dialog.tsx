...
>(({ className, children, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={clsx("fixed inset-0 z-50 bg-black/50 backdrop-blur-sm transition-opacity animate-in fade-in", className)}
    {...props}
    ref={ref}
  />
));
...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ref={ref}
      className={clsx(
        "fixed z-50 grid w-full max-w-lg scale-100 gap-4 bg-background p-6 opacity-100 animate-in fade-in-90 slide-in-from-bottom-10 sm:rounded-lg sm:zoom-in-90 sm:slide-in-from-bottom-0 md:w-full",
        "",
        className
      )}
      {...props}
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Title
    ref={ref}
    className={clsx("text-lg font-semibold text-foreground/90", className)}
    {...props}
  />
));
