type TwitterValidationResult = {
  valid: boolean;
  message: string;
}

export const validateTwitterUsername = (username: string): TwitterValidationResult => {
  if(username.length > 15) return {
    valid: false,
    message: "Username must be less than 15 characters"
  }
  else if(
    ["admin", "twitter"].some(reserved => username.toLocaleLowerCase().includes(reserved))
